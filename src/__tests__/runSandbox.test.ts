...
describe('kubernetes.runSandbox - Analytics Libraries Integration', () => {
  ...
  describe('ml-regression Integration', () => {
    it('can require and use ml-regression', async () => {
      const result = await runSandbox({
        code: `
          ...
          console.log('predict(10):', regression.predict(10));
        `,
      });
      ...
      expect(result.output).toContain('slope: 2');
      expect(result.output).toContain('intercept: 0');
      expect(result.output).toContain('predict(10): 20');
    });
    ...
    it('performs polynomial regression', async () => {
      const result = await runSandbox({
        code: `
          ...
          console.log('predict(6):', Math.round(prediction));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('predict(6): 36');
    });

    it('calculates R-squared score', async () => {
      const result = await runSandbox({
        code: `
          const { SimpleLinearRegression } = require('ml-regression');
          const x = [0, 1, 2, 3, 4, 5];
      ...
      expect(result.success).toBe(true);
      expect(result.output).toContain('r2: 1');
    });

    it('supports exponential regression', async () => {
      const result = await runSandbox({
        code: `
          const { ExponentialRegression } = require('ml-regression');
          const x = [0, 1, 2, 3, 4];
          const y = [1, 2.7, 7.4, 20.1, 54.6]; // approx e^x
          const regression = new ExponentialRegression(x, y);
          console.log('hasPredict:', typeof regression.predict === 'function');
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('hasPredict: true');
    });
  });

  describe('mathjs Integration', () => {
    it('can require and use mathjs', async () => {
      const result = await runSandbox({
        code: `
          const math = require('mathjs');
          console.log('mean:', math.mean([1, 2, 3, 4, 5]));
          console.log('median:', math.median([1, 2, 3, 4, 5]));
          console.log('std:', math.std([1, 2, 3, 4, 5]).toFixed(2));
        `,
      });
    ...
    it('performs matrix multiplication', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.output).toContain('[[19,22],[43,50]]');
    });

    it('calculates matrix inverse and determinant', async () => {
      const result = await runSandbox({
        code: `
          const math = require('mathjs');
          const A = [[4, 7], [2, 6]];
          console.log('det:', math.det(A));
          const inv = math.inv(A);
          console.log('inv[0][0]:', inv[0][0].toFixed(1));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('det: 10');
      expect(result.output).toContain('inv[0][0]: 0.6');
    });
    ...
    it('calculates eigenvalues', async () => {
      const result = await runSandbox({
        code: `
          const math = require('mathjs');
          // Use a symmetric matrix for more reliable eigenvalue computation
          const A = [[2, 1], [1, 2]];
          try {
            const eig = math.eigs(A);
            console.log('eigenvalues computed:', Array.isArray(eig.values.toArray()));
          } catch (e) {
            // eigs may not work in VM context
      ...
      expect(result.success).toBe(true);
      expect(['eigenvalues computed: true', 'eigs accessible']).toContain(result.output.trim());
    });
    ...
    it('performs element-wise operations', async () => {
      const result = await runSandbox({
        code: `
          ...
          const a = [1, 2, 3];
          const b = [4, 5, 6];
          console.log('add:', JSON.stringify(math.add(a, b)));
          console.log('dot:', math.dot(a, b));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('add: [5,7,9]');
      expect(result.output).toContain('dot: 32');
    });

    it('creates special matrices', async () => {
      const result = await runSandbox({
        code: `
          const math = require('mathjs');
          const eye = math.identity(3);
          const zeros = math.zeros(2, 3);
          console.log('identity:', JSON.stringify(eye.toArray()));
          console.log('zeros:', JSON.stringify(zeros.toArray()));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('[[1,0,0],[0,1,0],[0,0,1]]');
      expect(result.output).toContain('[[0,0,0],[0,0,0]]');
    });
  });

  describe('fft-js Integration', () => {
    it('can require and use fft-js', async () => {
      const result = await runSandbox({
        code: `
          const { fft, ifft, util } = require('fft-js');
          console.log('fft:', typeof fft);
          console.log('ifft:', typeof ifft);
          console.log('fftMag:', typeof util.fftMag);
          console.log('fftFreq:', typeof util.fftFreq);
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('fft: function');
      expect(result.output).toContain('ifft: function');
      expect(result.output).toContain('fftMag: function');
      expect(result.output).toContain('fftFreq: function');
    });

    it('computes FFT on a signal', async () => {
      const result = await runSandbox({
        code: `
          const { fft } = require('fft-js');
          const signal = [1, 0, 1, 0, 1, 0, 1, 0];
          const phasors = fft(signal);
          console.log('phasors length:', phasors.length);
          console.log('is array:', Array.isArray(phasors));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('phasors length: 8');
      expect(result.output).toContain('is array: true');
    });

    it('performs FFT and IFFT roundtrip', async () => {
      const result = await runSandbox({
        code: `
          ...
          // First value should be approximately 1
          console.log('first value:', Math.round(reconstructed[0][0]));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('first value: 1');
    });
    ...
    it('calculates magnitude spectrum', async () => {
      const result = await runSandbox({
        code: `
          ...
          // fftMag returns N/2 (half spectrum due to Nyquist)
          console.log('magnitudes received:', magnitudes.length >= 4);
          console.log('has positive values:', magnitudes.some(m => m > 0));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('magnitudes received: true');
      expect(result.output).toContain('has positive values: true');
    });
    ...
    it('calculates frequency bins', async () => {
      const result = await runSandbox({
        code: `
          const { fft, util } = require('fft-js');
          const signal = new Array(16).fill(0).map((_, i) => Math.sin(2 * Math.PI * i / 16));
          const phasors = fft(signal);
          const freqs = util.fftFreq(phasors, 16);
          // fftFreq returns N/2 frequency bins (half spectrum)
          console.log('freq bins received:', freqs.length >= 8);
          console.log('first freq:', freqs[0]);
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('freq bins received: true');
      expect(result.output).toContain('first freq: 0');
    });
  });

  describe('Multi-Library Integration', () => {
    it('combines simple-statistics and mathjs', async () => {
      const result = await runSandbox({
        code: `
          const ss = require('simple-statistics');
          const math = require('mathjs');
          ...
          console.log('ss.mean:', ss.mean(data));
          console.log('math.mean:', math.mean(data));
          console.log('equal:', ss.mean(data) === math.mean(data));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('ss.mean: 3');
      expect(result.output).toContain('math.mean: 3');
      expect(result.output).toContain('equal: true');
    });

    it('combines regression with statistics for residual analysis', async () => {
      const result = await runSandbox({
        code: `
          const ss = require('simple-statistics');
          const { SimpleLinearRegression } = require('ml-regression');
          ...
          const x = [1, 2, 3, 4, 5];
          const y = [2.1, 4.0, 6.2, 7.9, 10.1];

          const regression = new SimpleLinearRegression(x, y);
          const residuals = x.map((xi, i) => y[i] - regression.predict(xi));

          console.log('mean residual:', Math.abs(ss.mean(residuals)).toFixed(4));
          console.log('residual std:', ss.standardDeviation(residuals).toFixed(4));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('mean residual:');
      expect(result.output).toContain('residual std:');
    });

    it('combines FFT with matrix operations for signal analysis', async () => {
      const result = await runSandbox({
        code: `
          const math = require('mathjs');
          const { fft, util } = require('fft-js');

          // Generate sinusoidal signal using mathjs
          const n = 8;
          const signal = math.range(0, n).toArray().map(i =>
          ...
          // fftMag returns N/2 magnitudes (half spectrum)
          console.log('magnitudes computed:', magnitudes.length >= n / 2);
          console.log('max magnitude:', Math.max(...magnitudes).toFixed(2));
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('signal length: 8');
      expect(result.output).toContain('magnitudes computed: true');
    });

    it('performs complete time series analysis workflow', async () => {
      const result = await runSandbox({
        code: `
          const ss = require('simple-statistics');
          const { SimpleLinearRegression } = require('ml-regression');
          ...
          const values = [10, 12, 15, 14, 18, 20, 22, 21, 25, 28];

          // Calculate basic statistics
          const meanVal = ss.mean(values);
          const stdVal = ss.standardDeviation(values);

          // Fit trend line
          const regression = new SimpleLinearRegression(time, values);
          const trend = regression.slope;

          // Calculate moving average using mathjs
          const window = 3;
          const movingAvg = [];
          for (let i = window - 1; i < values.length; i++) {
            const slice = values.slice(i - window + 1, i + 1);
            movingAvg.push(math.mean(slice));
          }

          console.log('mean:', meanVal.toFixed(1));
          console.log('std:', stdVal.toFixed(1));
          console.log('trend:', trend.toFixed(2));
          console.log('moving avg points:', movingAvg.length);
        `,
      });

      expect(result.success).toBe(true);
      expect(result.output).toContain('mean:');
      expect(result.output).toContain('std:');
      expect(result.output).toContain('trend:');
      expect(result.output).toContain('moving avg points: 8');
    });
  });
  ...
  describe('Analytics Error Handling', () => {
      ...
      const result = await runSandbox({
        code: `
          ...
          try {
            ...
            math.multiply([[1, 2], [3, 4]], [[1, 2, 3], [4, 5, 6], [7, 8, 9]]);
          } catch (e) {
            console.log('caught dimension error:', e.message.includes('dimension'));
          }
        `,
      });
    ...
    it('handles insufficient data for regression', async () => {
      const result = await runSandbox({
        code: `
          const { SimpleLinearRegression } = require('ml-regression');
          try {
            // Need at least 2 points for linear regression
            const regression = new SimpleLinearRegression([1], [2]);
            console.log('slope:', regression.slope);
          } catch (e) {
            console.log('caught:', 'insufficient data');
          }
        `,
      });
